...
* By default the Jetty server runs as Moonshines deploy user (e.g. "rails").
* This plugin assumes that your Solr data is living in #{deploy_to}/shared/solr/data/#{Rails.env}, which is symlinked to #{Rails.root}/solr/data/#{Rails.env}. That means that your deploy user should have write access to this directory. This could be achieved by setting up a Capistrano task that chmods the Solr data directory for you, e.g.
    namespace :deploy do
