...
* This plugin assumes that your Solr data is living in <tt>#{deploy_to}/shared/solr/data/#{Rails.env}</tt>, which is symlinked to <tt>#{Rails.root}/solr/data/#{Rails.env}</tt>. That means that your deploy user should have write access to this directory. This could be achieved by setting up a Capistrano task that chmods the Solr data directory for you, e.g.
    namespace :deploy do
